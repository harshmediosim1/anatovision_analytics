...
def serve_layout():
    ...
    unique_users = [{'label': user, 'value': user} for user in df['user_id'].unique()] if not df.empty else []
    unique_modules = [{'label': module, 'value': module} for module in df['module'].unique()] if not df.empty else []
    unique_versions = [{'label': version, 'value': version} for version in df['version'].unique()] if not df.empty else []
    unique_locations = [{'label': location, 'value': location} for location in df['location'].unique()] if not df.empty else []
